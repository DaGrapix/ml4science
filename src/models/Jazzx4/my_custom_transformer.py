...
class PINN_head(torch.nn.Module):
    ...
    def __call__(self, input, models, skeleton_features, scaler) -> Tensor:
        """
        Returns the most physical output from the models
        Args:
            input: (N, 4) tensor: input data
            models: list[torch.nn.Module]: list of models
            skeleton_features: (N, 4) tensor: the skeleton features
        """
        N = input.shape[0]

        std_x, std_y = torch.tensor(scaler._std_x[0]).to(self.device).requires_grad_(False), torch.tensor(scaler._std_x[1]).to(self.device).requires_grad_(False)
        mean_x, mean_y = torch.tensor(scaler._m_x[0]).to(self.device).requires_grad_(False), torch.tensor(scaler._m_x[1]).to(self.device).requires_grad_(False)
        ...
        remaining_inputs = input[:, 2:].detach().to(self.device)
        input = torch.cat([x, y, remaining_inputs], dim=1)

        # computing the most physical loss
        output_lists    = torch.empty(N, len(models), 4).to(self.device)
        loss_list       = torch.empty(N, len(models)).to(self.device)
